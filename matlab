...
if nargin < 6 || isempty(tle_filename)
    tle_filename = 'C:\Users\HP\Desktop\tlesat.txt';
end

if nargin < 5 || isempty(orb_filename)
    fid_orb = 1;
else
    fid_orb = fopen(orb_filename, 'wt');
end

fid_tle = fopen(tle_filename, 'rt');
if fid_tle == -1
    error('TLE file could not be opened');
end
...
% read TLE from the file
tle_lines = textscan(fid_tle, '%s', 'Delimiter', '\n');
tle_lines = tle_lines{1};
fclose(fid_tle);

if length(tle_lines) < 2
    error('Not enough TLE lines in the file');
end

longstr{1} = tle_lines{1};
longstr{2} = tle_lines{2};
...
while (tsince < stopmfe) && (satrec.error == 0)
    ...
    if satrec.error ~= 0
        ...
        if any([orb_out.year(idx), orb_out.month(idx), orb_out.day(idx), orb_out.hour(idx), orb_out.min(idx)] ~= [year, mon, day, hr, minute]) || abs(orb_out.sec(idx) - sec) > 1e-6
            error([mfilename, ': discrepancy in date/time conversion from different algorithms']);
        end
